  ...
  export default function App() {
    ...
    async function loadAuthState() {
      try {
        const response = await fetch("http://localhost:7860/api/auth/me");
    ...
    async function loadSavedSessions() {
      try {
        const response = await fetch("http://localhost:7860/api/sessions");
    ...
    async function startInterview() {
      ...
      try {
        ...
        const res = await fetch("http://localhost:7860/api/start-interview", {
          method: "POST",
          body: form,
        });
    ...
    async function renderAIMessage(text: string) {
      ...
      // Show the message progressively, matching the original word-sync behavior.
      ...
      for (let i = 0; i < words.length; i++) {
        ...
        setMessages((prev) => {
          ...
          if (last?.role === "assistant" && last.content.startsWith("__SPEAKING__")) {
            copy[copy.length - 1] = { role: "assistant", content: streamed };
            return copy;
          }
          return [...copy, { role: "assistant", content: streamed }];
        });
    ...
    async function speakText(text: string) {
      try {
        const res = await fetch("http://localhost:7860/api/synthesize", {
          method: "POST",
    ...
    async function processAudio() {
      ...
      try {
        const tr = await fetch("http://localhost:7860/api/transcribe", { method: "POST", body: fd });
        ...
        const rr = await fetch("http://localhost:7860/api/respond", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    async function viewSavedSession(interviewId: string) {
      try {
        const response = await fetch(`http://localhost:7860/api/sessions/${encodeURIComponent(interviewId)}`);
        if (!response.ok) return;
    ...
    async function endAndGetFeedback() {
      ...
      try {
        const response = await fetch("http://localhost:7860/api/end-interview", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
                      ...
                      <div className="nofox-scroll flex flex-1 flex-col gap-5 overflow-y-auto p-5">
                        ...
                        {isSpeaking && (
                          <div className="flex items-center gap-3">
                            <div className="flex h-7.5 w-7.5 items-center justify-center rounded-full border border-[rgba(189,48,40,.2)] bg-[rgba(189,48,40,.08)]">
                              🤖
                            </div>
                            <div className="flex items-center gap-0.75 rounded-[14px] rounded-tl border border-[rgba(73,38,31,.14)] bg-[#f0e7df] px-3.5 py-2.5">
